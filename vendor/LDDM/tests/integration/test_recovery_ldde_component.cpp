...
TEST_CASE(Recovery_LddeComponentRecovery) {
    std::string base_dir = "/tmp/lddm_test_rec_ldde_comp";
    ...
    // Kill LDDE process
    ::kill(orig_ldde_pid, SIGKILL);
    std::this_thread::sleep_for(std::chrono::milliseconds(50));

    // Execute LDDE recovery
    auto rec_res = rec_mgr->recover(RecoveryReason::LddeUnexpectedExit, "ldde");
    EXPECT_TRUE(rec_res.has_value());
    EXPECT_TRUE(session.is_graphical_session_ready());
    EXPECT_EQ(session.state(), SessionState::RUNNING);

    // Weston PID was not restarted
    EXPECT_EQ(weston_mgr->process()->pid(), orig_weston_pid);
    // LDDE was restarted with new PID
