...
TEST_CASE(Recovery_WestonComponentRecovery) {
    std::string base_dir = "/tmp/lddm_test_rec_weston_comp";
    ...
    // Kill Weston process (simulating unexpected crash)
    ::kill(orig_weston_pid, SIGKILL);
    std::this_thread::sleep_for(std::chrono::milliseconds(50));

    // Execute Weston recovery
    auto rec_res = rec_mgr->recover(RecoveryReason::WestonUnexpectedExit, "weston");
    EXPECT_TRUE(rec_res.has_value());
    EXPECT_TRUE(session.is_graphical_session_ready());
    EXPECT_EQ(session.state(), SessionState::RUNNING);

    // Both Weston and LDDE were restarted with fresh PIDs
